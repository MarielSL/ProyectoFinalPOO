...
public class ReporteEmpresas extends JDialog {
	...
	public static void main(String[] args) {
		try {
			ReporteEmpresas dialog = new ReporteEmpresas();
	...
	public ReporteEmpresas() {
		...
		JLabel lblTitulo = new JLabel("Resumen de empresas asociadas:");
		lblTitulo.setHorizontalAlignment(SwingConstants.CENTER);
		lblTitulo.setFont(new Font("Calibri", Font.BOLD, 16));
		contentPanel.add(lblTitulo, BorderLayout.NORTH);

		lblTotalEmpresas = crearLabelValor(new Color(0, 120, 0));
		lblEmpresasActivas = crearLabelValor(new Color(204, 102, 0));

		JPanel panelStats = new JPanel();
		panelStats.setLayout(new GridLayout(1, 2, 20, 0));
		panelStats.add(crearBloque("Total de empresas", lblTotalEmpresas));
		panelStats.add(crearBloque("Empresas activas", lblEmpresasActivas));
		...
		SwingWorker<int[], Void> hilo = new SwingWorker<int[], Void>() {
			...
			protected void done() {
				try {
					...
					lblTotalEmpresas.setText(String.valueOf(datos[0]));
					lblEmpresasActivas.setText(String.valueOf(datos[1]));
					...
					lblTotalEmpresas.setText("0");
					lblEmpresasActivas.setText("0");
					lblEstado.setText("No se pudieron cargar los datos");

					JOptionPane.showMessageDialog(ReporteEmpresas.this,mensaje != null ? mensaje : "No se pudo generar el reporte de empresas.","Error",JOptionPane.ERROR_MESSAGE);
